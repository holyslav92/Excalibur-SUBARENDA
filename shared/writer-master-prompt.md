...
- **Воронка в теле (HARD):** после чеклиста заселения → TG канал «полный список»;
  после бренд-блока «у нас так» → MAX или менеджер (инструкция до заселения)
- **Dzen feed (5 паттернов):** см. Scout `dzen_pattern` + `shared/article-style.md` —
  выполни обещание H1 (список с числом, кейс с суммами, страх→инструкция в §1,
  контраст с ответом в лиде, локальный+сезонный крючок). `dzen-excerpt.json` для карточки.
- **Дзен — поверхность дистрибуции:** статья на сайте стоит сама и конвертит в TG/MAX.
- **Бан:** ЕГРН, нотариус, суд, «я адвокат», «мы лучшие», бизнес-класс, WhatsApp, чужие телефоны
