...
> **TENANT:** Добрый дом / добрыйдом-72.рф — `memory/cover/*`, `shared/tenant-config.json`.
...
1. **Изобретение с нуля** — no inventory lock, no default recurring props (keys, hologram, desk, balcony, EGRN board).
2. **Anti-repeat 14д** — `memory/cover/used-motifs.json` + `excalibur_blog_cover_motif_gate.py check` перед `--write-batch`.
3. **Light & bright** — high-key, sun flare, teal curtains + terracotta accents; dark cinematic запрещён.
4. **Logo lockup** — `memory/cover/assets/brand/logo-dobry-dom.png` на **всех 8** изображениях.
5. **NO host face / NO Shakin** — люди по теме OK, identity lock запрещён.
6. **Мемы** — meme cats + meme people stickers; 8-set utility inlines.
7. **Wordstat stickers** — 1–3 readable stickers с live Wordstat фразами (Тюмень/область, regions 55+11176).
8. **REJECTED daypart formula** — never: morning desk+document / day street / evening close talk / night split.

## Brand logo lockup

- `memory/cover/assets/brand/logo-dobry-dom.png` — official logo (green curtain mark + terracotta «Добрый дом»)
- `blog-hero.json` → `reference_url_hosted` for Derouter `input_urls`
- Logo on cover + all 7 inlines — consistent corner lockup, readable, not giant watermark
...
- Панели `#FFFFFF` high-key; ink `#141821`; terracotta `#c45c3e` + teal `#3d8b7a`; sun flare/glow OK
- **Cover:** warm hospitality scene, invented each run, logo lockup, Wordstat stickers, meme stickers
- **Inline (7 шт.) — UTILITY-FIRST** (канон: `memory/cover/inline-visual-types.json`):
  - Стиль = одобренная обложка: белый high-key, terracotta/teal, скотч, рваная бумага, солнце, коллаж
  - **Logo lockup** на каждом inline
  - **Без locked host face / Shakin** — identity lock запрещён
  - **Тест пользы (FAIL):** decorative-only interior; ряд иконок + 3 слова = FAIL
...
- Запреты inline: icon row+caption; дубли подписей; decorative-only; пустой UI; копипаст cover; Salt Bae/Drake/celebrity
- Запреты общие: dark cinematic, daypart formula, inventory default props, Shakin face, empty doc-only office

## Blockers

- нет logo / reference / canvas 1 без `input_urls`
...
- logo missing on any of 8 images
