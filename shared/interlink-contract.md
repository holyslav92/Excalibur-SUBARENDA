...
1. **Outbound (новая статья)** — Writer/Sol добавляют 1–3 контекстные ссылки на
   ...
   `status=published`) **и/или** slug из кэша `memory/live-catalog.json`
   (ledger lag: live WP посты ещё не в ledger). Якорь — по смыслу H2, не «читайте также» в каждом абзаце.
