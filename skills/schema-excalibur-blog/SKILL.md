...
6. **Publish slug (HARD, INC B12):** `<slug>` = `title-brief.json` → `article.meta.json` → **никогда** только суффикс `--article-dir`, если Title выдал другой slug.  
   ...
   В `assembled-schema-inputs.md` canonical URL строить только из publish slug (`python3 -c "from scripts.excalibur_blog_article_meta_index import resolve_publish_slug; print(resolve_publish_slug(...))"`).
