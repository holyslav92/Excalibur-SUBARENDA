...
def uses_brand_logo_paste(cfg: dict[str, Any]) -> bool:
    ...
    if mode in {"full_grsai_cover", "grsai_full_cover", "one_2k_slice4", "gen_only_slice4"}:
        return True
...
def composite_article_images(
    ...
    *,
    ...
    if manifest_path.is_file():
        try:
            from excalibur_blog_quad_slots import inline_count_from_manifest
    ...
    if cover_only:
        ...
        targets = ("cover.png",) + tuple(f"inline-{i:02d}.png" for i in range(1, inline_count + 1))
    for name in targets:
...
def validate_logo_stamp(article_dir: Path, root: Path) -> list[str]:
    ...
    for name in ("cover.png",) + tuple(f"inline-{i:02d}.png" for i in range(1, 8)):
        path = article_dir / "cover" / name
        if not path.is_file():
            if name == "cover.png":
