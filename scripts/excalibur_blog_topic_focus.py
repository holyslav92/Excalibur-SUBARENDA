...
# Добрый дом / посуточная аренда и субаренда — не сделки с авансом.
SHORT_TERM_RENTAL_ALLOW_PATTERNS: tuple[str, ...] = (
    r"посуточн",
    r"субаренд",
    r"аренд",
    r"квартир",
...
def allow_patterns_for_tenant() -> tuple[str, ...]:
    ...
    if profile in {
        ...
        return SHORT_TERM_RENTAL_ALLOW_PATTERNS
    if profile in {"real_estate", "realestate", "rieltor"}:
        return REAL_ESTATE_ALLOW_PATTERNS
    if "посуточн" in niche or "субаренд" in niche:
        return SHORT_TERM_RENTAL_ALLOW_PATTERNS
...
def core_focus_hint() -> str:
    if allow_patterns_for_tenant() is SHORT_TERM_RENTAL_ALLOW_PATTERNS:
        return (
            "short-term rental / sublease Tyumen (check-in/deposit/neighbors/"
            "utilities/hidden fees/rules)"
        )
