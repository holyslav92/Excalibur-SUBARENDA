...
def parse_image_response(parsed: dict[str, Any], *, download_timeout: int = 180) -> bytes:
    """Derouter images API returns data[0].b64_json (PNG); Grsai VIP may return data[0].url."""
    ...
    if url:
        request = urllib.request.Request(
            url,
            headers={"Accept": "image/*", "User-Agent": "ExcaliburBlogDerouter/1.0"},
            method="GET",
        )
        try:
