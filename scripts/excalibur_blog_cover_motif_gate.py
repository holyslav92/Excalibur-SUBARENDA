...
def normalize_motif_field(field: str, value: str, root: Path) -> str:
    """Cat-meme family: любой кот = один токен для anti-repeat 14д."""
    ...
    if field != "meme":
        return normalized
    try:
        from excalibur_blog_meme_cat_gate import load_meme_catalog, normalize_meme_for_antirepeat

        catalog = load_meme_catalog(root)
        return normalize_meme_for_antirepeat(normalized, catalog)
    except ImportError:
        return normalized
...
def motif_payload_from_args(args: argparse.Namespace, root: Path) -> dict[str, str]:
    fields = ("composition", "location", "meme", "prop_set", "sticker_set", "joke")
    payload: dict[str, str] = {}
    for field in fields:
        raw = getattr(args, field, "") or ""
        value = normalize_motif_field(field, raw, root)
        if value:
            payload[field] = value
    return payload
    ...
    *,
    ...
    for entry in entries:
        ...
        for field, value in motifs.items():
            prior_value = normalize_motif_field(field, str(prior.get(field) or ""), root)
...
def cmd_check(root: Path, args: argparse.Namespace) -> int:
    ...
    motifs = motif_payload_from_args(args, root)
    if not motifs:
        print("FAIL motif gate: pass at least one --composition/--location/--meme/--prop-set/--sticker-set/--joke", file=sys.stderr)
        return 1

    collisions = find_collisions(motifs, entries, topic_id=args.topic_id or "", root=root)
    if collisions:
